...
De plus, le développement \flqq{}\emph{test d'abord}\frqq{} (\ct{test-first}) 
...
journal (en anglais, \emph{un log}) plus détaillé des
résultats en lançant  l'\emphind{exécuteur de tests}  \emphind{Test
  Runner} de \sunit depuis le menu \menu{World \go Test Runner}.
L'exécuteur de tests, montré dans la \figref{test-runner}, est conçu pour faciliter 
...
Comme montré dans \figref{sunit-classes}, \sunit consiste en quatre classes 
...
passés, le nombre de tests en échec et le \arelire{nombre d'erreurs signalées.} % CHANGE
...
%---------------------------------------------------------
\subsection{Description des assertions avec des chaînes \arevoir{de caractères}} % CHANGE
...
La mise en {\oe}uvre de \sunit constitue un cas d'étude intéressant d'un \emph{framework} \st.
