...
%
% adaptation pour PBE - martial - Thu Sep 10 20:18:24 CEST 2009 from
...
Le modèle de programmation de \st est simple et homogène: \mantra et les objets communiquent les uns avec les autres uniquement via l'envoi de messages.
Cependant, ces caractéristiques de simplicité et d'homogénéité peuvent être source de quelques difficultés pour le programmeur habitué à d'autres langages. Dans ce chapitre nous présenterons les concepts de base du modèle objet de \st; en particulier nous discuterons des conséquences de représenter les classes comme des objets.
...
Nous disons que l'\emphind{espace d'encapsulation}~\footnote{En anglais, encapsulation boundary.} des objets en Java et en C++ est la classe, là où, en \st, c'est l'instance.
...
En \st, deux instances d'une même classe ne peuvent pas accèder aux variables d'instance l'une de l'autre à moins que la classe ne définisse des ``méthodes d'\ind{accès}'' (en anglais, accessor methods).
...
La méthode \cmind{Point}{dist:} (\mthref{dist:}) calcule la distance entre le receveur et un autre point. Les variables d'instance \ct{x} et \ct{y} du receveur sont accédées directement par le corps de la méthode. Cependant, les variables d'instance de l'autre point doivent être accédées en lui envoyant les messasges \ct{x} et \ct{y}.
...
Toutes les méthodes sont \subind{méthode}{publique}{}s~\footnote{En fait, presque toute.  En \squeak, des méthodes dont les sélecteurs commencent par
...
par exemple, \protind{accessing} pour les méthodes d'accès, et \protind{initialization} pour construire un état initial stable pour l'objet.
...
n'étaient qu'une seule entité avec deux ``côtés'': le ``\subind{Browser}{côté instance}'' et le ``\subind{Browser}{côté classe}'', comme le montre la \figref{Buttons}. 
\damien{something wrong here. The text between quotes is not generated}
% true: \ind should replace \index there! (martial)
En cliquant sur le 
bouton \button{instance}, vous voyez la présentation de la classe \ct{Color}, \ie vous pouvez naviguer dans les méthodes qui sont exécutées quand
les messages sont envoyés à une instance de
\ct{Color}, comme la couleur blue (correspondant au bleu). En appuyant
sur le bouton \button{class} (pour classe), vous naviguez dans la classe \ct{Color class}, autrement dit vous voyez les méthodes qui seront exécutées
en envoyant les messages directement à la classe \ct{Color} elle-même.
Par exemple, \ct{Color blue} envoie le message \ct{blue} (pour \emph{bleu}) à
...
Vous pouvez naviguer dans la méta-classe en cliquant sur le bouton \button{class}.
...
Ceci est courant, bien que vous trouverez occasionnellement des méthodes
de classe utilisées d'une autre manière.
...
sans avoir à créer un objet additionnel dans un premier temps. % coutumier -> commun
...
\dothis{Naviguez dans la méthode \ct{Color class>>>showColorCube}, double-cliquez à l'intérieur des guillements englobant le commentaire \ct{"Color showColorCube"} et tape au clavier \short{d}.}
...
toutes les instances d'une classe partagent le même ensemble
de noms de variable et les instances de ses sous-classes héritent
de ces noms; cependant, chaque instance possède son propre jeu de valeurs.
...
Tout ce que peut faire la classe, c'est d'envoyer des messages d'\ind{initialisation} à des instances nouvellement créées; ces messages pouvant bien sûr utiliser les méthodes d'accès.
...
Imaginez que nous souhaitons d'une part, créer une classe \ct{WebServer} et d'autre part, s'assurer qu'il n'a qu'une et une seule instance en faisant appel au patron Singleton.

En cliquant sur le bouton \button{instance} dans le navigateur de classe, nous définissons la classe \clsind{WebServer} comme suit (\clsref{singleton}).
...
Ensuite, en cliquant sur le bouton \button{class}, nous pouvons ajouter une variable d'instance \ct{uniqueInstance} au \subind{Browser}{côté classe}.
...
Par conséquence, la classe \ct{WebServer} a désormais un autre variable d'instance, en plus des variables héritées telles que \ct{superclass} et \ct{methodDict}.
...
% proposition de Rene :
% Ces deux variables ont, bien entendu, des valeurs différentes.
\arevoir{Ces deux variables ont, bien entendu, différentes valeurs.}
...
Bien que \pharo ne dispose pas d'héritage multiple, il 
\arelire{dispose d'un}
mécanisme appelé % CHANGE 
...
Si nous instancions maintenant \ct{LOGame}, l'instance répondra comme prévu au message \ct{author}.
...
des messages, puisque les classes sont aussi des objets. % REVOIR 'by
                                % sending messages'
...
la classe du receveur cherche la méthode à utiliser pour
opérer le message.
...
\begin{itemize}
  ...
  \item \emph{Qu'elle différence y a-t-il entre les envois faits à \pvind{self} et ceux faits à \pvind{super}?}
...
C'est leur travail mais tout est fait pour que vous ne remarqueriez jamais qu'elles font quelque chose de différent des règles énoncées.
...
Tout d'abord, penchons-nous sur la stratégie de base de la recherche. Ensuite
...
En fait, si nous ouvrons un nouvel élément graphique \emph{Morph} (\ct{Morph new openInWorld}), nous constatons que nous obtenons un morph bleu, là où l'ellipse (\ct{EllipseMorph}) est jaune (yellow) par défaut.
...
de la chaîne d'héritage fournissent déjà la connaissance
nécessaire. 
La sous-classe n'a pas à s'occuper d'initialiser les variables d'instance héritées!

C'est une bonne pratique d'envoyer \ct{super initialize} avant tout autre considération lorsque nous créons une méthode d'\ind{initialisation}:
\arevoir{}% subpvindex
...
\end{code}
La valeur de retour est une chaîne de caractères qui peut être
évaluée pour recréer un morph.

Comment ce résultat est-il exactement obtenu grâce à l'association
de \self et de \super?
Pour commencer, 
...
Ce message est \arevoir{recherché} dans la hièrarchie en commençant dans la classe
...
Encore une fois, \ct{fullPrintOn:} est \arevoir{recherché}
depuis la classe
\ct{EllipseMorph} et \mthind{BorderedMorph}{fullPrintOn:} est
trouvée dans \ct{BorderedMorph} (revoir
\figref{constructorStringLookup})} % CHANGE
...
Supposons que nous envoyions le message \ct{foo} à une ellipse \ct{anEllipse}.
...
Il apparaît que \ct{Object} implémente \ct{doesNotUnderstand:}.
...
qu'il ne comprend pas à un autre objet responsable de les gérer ou
...
La variable \glbind{Transcript} nomme une instance de \clsind{TranscriptStream}, un flux de données ou \emph{stream} qui écrit dans une fenêtre 
à ascenseur (dite aussi \emph{scrollable}).
...
\noindent
Avant vous lanciez la commande \menu{do it}, ouvrez un Transcript en 
\arelire{sélectionnant \menu{World \go Tools \ldots \go Transcript}.} % CHANGE
...
\item \clsind{Sensor} est une instance of \clsind{EventSensor}. Il représente les entrées interactives ou intefaces de saisie (en anglais, \emph{input}) dans \pharo.
Par exemple, \ct{Sensor keyboard} retourne le caractère suivant saisi au clavier, et \ct{Sensor leftShiftDown} répond \ct{true} (vrai en booléen) si la
touche \emph{shift} gauche est maintenue enfoncée, alors que \ct{Sensor mousePoint} renvoie un \ct{Point} indiquant la position actuelle de la souris.
...
Cependant, si par la suite, vous effacez la déclaration, le code
référencera une variable non déclarée. 
...
Une \subind{classe}{variable} de classe est déclarée dans le patron
de définition de la classe.
Par exemple, la classe \ct{Color} définit un grand nombre de variables
de classe pour accélérer la création des couleurs;
...
couleurs fréquemment utilisées. Ce tableau est partagé par toutes les instances de \ct{Color} et de sa sous-classe \clsind{TranslucentColor}. 
...
en regroupant ces dernières en \emph{traits}.
  ...
  \item Tout se passe par envoi de messages. % REVOIR 'by sending
	...
	Nous ne faisons pas des ``appels de méthodes'', nous
...
Les variables de classe sont partagées entre une classe, ses sous-classes et
ses instances.
