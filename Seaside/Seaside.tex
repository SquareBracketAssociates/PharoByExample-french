...
% 2011-05-04 - Rene mages finished the fourth rereading
% 2011-06-15 - Rene mages finished the last rereading
...
Les applications web modernes essaient d'interagir avec l'utilisateur
...
Dans le coin supérieur droit, le 
 ...
 code rendu et celle du code source.
...
\dothis{Pointez votre navigateur web
  ...
  entrée qui vous appellerez ``hello'' et choisissez \ct{WAHelloWorld}
...
Le rendu de ce composant s'effectue en faisant le rendu de ses
...
de \brushes simples inclus dans
Seaside\footnote{Le code source de
  \mthref{renderdemo} se trouve dans le paquetage \ct{PBE-SeasideDemo}
  dans le projet \url{http://www.squeaksource.com/PharoByExample}.
  \frsays{NdT: Les textes ont été traduits dans le présent ouvrage.
    Attendez-vous à ce que la version du code d'exemple que vous téléchargerez soit
      en anglais.}{}}.
...
Dans l'exemple, ce message est utilisé pour attacher une zone de saisie
...
Vous devriez alors définir des noms symboliques pour ces
...
\emph{déclaration} qui liste un certain nombre d'attributs pour cet ou
ces éléments.
...
\emph{nouveau} nombre ou que vous complétez un nombre existant.
...
Nous sommes maintenant capables d'adjoindre du code \jscript aux
...
sans AJAX\,---\,de l'application RPN fonctionnent.
...
nom descriptif pour l'application qui sera affichée dans l'éditeur de
configuration}; % ATTENDRE - martial: pas vu du tout, ça!
  ...
  \arevoir{Le message \ct{with:} envoie le contenu \emph{et} retourne le rendu du résultat;}
...
\ct{call:} une série de composants dans une séquence de tâches;
  ...
  \item Pour interdire à l'utilisateur de se servir du bouton \backbtn
    de son navigateur web pour accéder à un état d'exécution passé de
