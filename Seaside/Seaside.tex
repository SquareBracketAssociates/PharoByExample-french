...
\arelire{Une table utilise deux niveaux de blocs pour afficher chacune
...
suivants, nous avons \arelire{souligné les messages \ct{call:} et
\ct{answer:} pour plus de clareté}. % REVOIR - un peu changé par
                                % rapport à la VO
...
fonctionnera bien (bien que les \arelire{effets de bord ne soient pas
rembobinés au moins d'avoir pris des dispositions complémentaires}).
...
clique sur ``ok''.
\arelire{Le composant appelé retourne \ct{self}.}
...
méthode \ct{states} pour répondre un tableau de toutes les objets dont
...
\dothis{\,Achetez avec la \arevoir{commande ``Checkout'' des sushis,
...
\arelire{Vous aurez le message ``That page has expired'' (\ie{} ``la
  page a expiré'').}}
...
Nous allons construire une calculatrice RPN~\footnote{La RPN pour
  ...
  \emph{postfixe}.} % martial - perso, c'est ma notation favorite (PS,
                    % factor, forth...)
comme une application Seaside qui \arelire{utilise une machine à pile simple
comme modèle (``pile'' en anglais se disant ``\emph{stack}'')}.
%simple stack machine as its underlying model.
...
de la pile.
%the other which shows us the complete state of the stack.
...
commandes pour
voir le sommet de la pile que nous appellerons \ct{top} et pour faire
...
% ajout - vf (plus clair)
de la partie \ct{display}
de la page existante.
...
L'argument de bloc \ct{r} est \arelire{une nouvelle inteface de rendu ou
...
Si vous surchargez \ct{description}, il est possible de retourner 
% it is possible to return a descriptive application name that will be displayed in the configuration editor.
