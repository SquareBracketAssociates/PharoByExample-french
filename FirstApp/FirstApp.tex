...
travail. \arelire{C'est très utile car, dans cet espace de travail, la
...
\arelire{%
 La variable \ct{bounds} devrait changer
dans l'inspecteur. Saisissez maintenant \ct{self openInWorld} dans
...
conséquence \arelire{(vous pourriez avoir à \actclick{} sur
  \menu{refresh} pour voir les nouvelles valeurs)}. % CHANGE
...
\arelire{\ldots{} ou presque! S'il vous arrive de bouger la souris
...
\actclickant{} dans le panneau des paquetages vous permet de
générer un fichier correspondant au paquetage
\scat{PBE-LightsOut} tout entier via l'option
\menu{various \go{} file out}.} % CHANGE ATTENDRE REVOIR : devrait être des packages
