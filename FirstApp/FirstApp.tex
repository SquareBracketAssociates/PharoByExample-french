...
% adaptation pour PBE - martial - (09/20/2009) version 29170
...
Dans ce chapitre, nous allons développer un jeu simple \arelire{de
...
En cours de route, nous allons faire la démonstration de la plupart des outils que les développeurs \pharo utilisent pour construire et déboguer leurs programmes et comment les programmes sont échangés entre les développeurs. Nous verrons notamment le navigateur de classes, l'inspecteur d'objet, le débogueur et le navigateur de \ind{paquetage}{}s \ind{Monticello}. 
...
Ceci est en partie du au fait que Smalltalk est un langage très simple, et d'autre part que les outils qui forment l'environnement de programmation sont très intégrés avec le langage.
...
Le jeu Lights Out montré dans \figref{gameBoard} est fait de deux types d'objets: le plateau de jeu lui-m\^eme et une centaine de cellule-objets individuelles. Le code \pharo pour réaliser ce jeu va contenir deux classes: une pour le jeu et une autre pour les cellules.
...
\dothis{\arelire{Ouvrez un Browser et \actclickz{} sur le panneau des paquetages.
Sélectionnez \menu{create package}\footnote{Nous supposons que le
  Package Browser est installé en tant que navigateur de classes par
  défaut. Si le Browser ne ressemble pas à celui de la
...
\menu{accept} (ou appuyez simplement sur la touche entrée); \arelire{le nouveau
paquetage est créé et se positionne dans la liste des paquetages en
respectant l'ordre alphabétique}.

%=================================================================
\section{Définir la classe LOCell}

\arelire{Pour l'instant, il n'y a aucune classe dans le nouveau paquetage. Cependant le
...
\subsection{\arelire{Des catégories et des paquetages}}
...
raccourci-clavier \short{s} (pour
``save'' 
%ajout
\cad sauvegarder).
...
nouvel objet \ct{LOCell} et en l'inspectant
% ajout - vf
\arelire{avec l'inspecteur nommé Inspector}.
...
travail. \aretirer{C'est très utile car, dans cet espace de travail, la
  pseudo-variable \self est liée à l'objet inspecté.} % REVOIR
                                % ATTENDRE dans PBE la phrase existe
                                % mais c'est faux!?


% \dothis{Tapez le texte \ct{self openInWorld} dans la zone du bas et
%  choisissez \menu{do it} via le menu.}
\dothis{\arelire{Séléctionnez LOCell à la racine de la fenêtre de
...
\arelire{%
 Si   vous inspectez la variable \ct{bounds}, vous devriez constater que
  sa valeur a changé. Saisissez maintenant \ct{self openInWorld} dans
...
\arelire{Pour cela, \clickz{} sur le nom du paquetage.}
...
catégoriser les classes \arelire{dans des paquetages} de telle sorte
...
\begin{figure}[htbp]
   ...
   \caption{Catégoriser \arelire{automatiquement} toutes les méthodes
...
\pharo ne peut pas le savoir mais dans notre cas, il y a aussi une méthode \ct{initialize} dans la super-classe et \pharo suppose que notre méthode \ct{initialize} doit \^etre rangée dans la m\^eme catégorie que celle qu'elle surcharge.
...
Définissons maintenant les autres méthodes qui sont utilisées par la méthode \ct{LOGame>>>initialize}. Les deux peuvent \^etre mises dans le protocole \prot{initialization}.
...
Afin de compléter le jeu Lights Out, nous avons besoin de définir encore deux méthodes dans la classe \ct{LOCell} pour de gérer les événements souris.
...
Dans notre cas, vous pouvez voir dans la première ligne du panneau du haut que le message \ct{toggleState} a été envoyé à une instance de \ct{LOGame}, alors qu'il était clairement destiné à une instance de \ct{LOCell}.
...
Par défaut, une méthode retourne \ct{self}, ce qui dans le cas de \ct{newCellAt:at:} est effectivement une instance de \ct{LOGame}.
...
sortie-fichier (\emph{filing out}) de votre code. \arevoir{Le menu activé en
\actclickant{} dans le panneau de \aretirer{catégories} vous permet de
générer un fichier correspondant à l'ensemble de la \aretirer{catégorie}
\scat{PBE-LightsOut}.} % CHANGE ATTENDRE REVOIR : devrait être des packages
...
\dothis{%
\arelire{\Actclickz{} sur le paquetage
  \scat{PBE-LightsOut} et choisissez \menu{various \go{} file out} pour exporter le contenu.}}
Vous devriez trouver maintenant un fichier PBE-LightsOut.st dans le m\^eme
...
\dothis{Ouvrez une nouvelle image \pharo et utilisez l'outil \arelire{File
...
partagent le m\^eme préfixe. Comme nous avons mis tout le code du jeu
Lights Out dans la catégorie de classes appelée \scat{PBE-LightsOut},
nous pouvons le désigner comme le paquetage \ct{PBE-LightsOut}.} % REVOIR bizarre avec la sous-section 'Des categories et des paquetages'
...
\arelire{avec le bouton \button{Open} ou en \actclickant{}
% ajout - vf
...
et des méthodes. Nous avons vu aussi comment utiliser le \arelire{navigateur de
