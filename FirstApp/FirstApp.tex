...
% relecture: Rene Mages  (11/04/2011) 
% relecture: Rene Mages  (01/05/2011)
...
En cours de route, nous allons faire la démonstration de la plupart des outils que les développeurs \pharo utilisent pour
...
Browser est installé en tant que navigateur de classes par défaut. 
...
% rene : Voyez la FAQ ??, p. ??.  donc probleme avec \faqref
...
dire:  ``Utilise des bords carrés!'' (d'où \mbox{\ct{useSquareCorners}),}
...
nouvel objet \ct{LOCell} et en l'inspectant avec l'inspecteur nommé Inspector.
...
Si vous avez suivi l'exemple jusqu'à présent, le troisième panneau doit contenir le protocole \protind{as yet unclassified}~\footnote{NdT: pas encore classées.}. % ie méthodes pas encore classées
...
\Tmthref{newCellAt:at:} retourne une nouvelle cellule \ct{LOCell} à la position \mbox{\ct{(i,j)}{}} dans la matrice (\clsind{Matrix}) de cellules.
...
\emph{méthode d'}\emphsubind{méthode}{accès en écriture}
...
d'instance est appelée une \emph{méthode d'}\emphsubind{méthode}{accès en lecture} ou
\emph{accesseur} (le mot anglais équivalent est \emph{getter}).
\seeindex{méthode d'accès}{méthode, accès}
\seeindex{accesseur}{méthode, accès en lecture}
\seeindex{mutateur}{méthode, accès en écriture}
\seeindex{méthode!getter}{méthode, accès en lecture}
\seeindex{méthode!setter}{méthode, accès en écriture}

Si vous êtes habitués aux méthodes d'\subind{méthode}{accès en lecture} (\emph{getter})
et \subind{méthode}{accès en écriture} (\emph{setter}) dans d'autres langages de programmation,
...
Une méthode d'\subind{méthode}{accès} (en lecture ou en écriture) est appelée
en anglais \emphind{accessor} et par convention, elle doit être
placée dans le protocole \mbox{\protind{accessing}{}.}
...
\dothis{Ouvrez une nouvelle image \pharo et utilisez l'outil File
  ...
  de fichier via l'option de menu  \menu{file in} du fichier
