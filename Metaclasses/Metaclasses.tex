...
% relecture : Rene Mages 15-06-2011
...
\end{code}
\noindent
Vous pouvez voir l'effet produit dans \figref{translucentEllipse}.
...
%% ajout
entre autres la super-classe, les variables d'instances, le \subind{méthode}{dictionnaire} des méthodes 
%% etc.
...
En réalité, vous verrez que nous avons dit un pieux mensonge\,---\,\lct{Object class superclass}{}\mbox{\ct{-->}~\clsind{ProtoObject class},} et non \ct{Class}. En \pharo, il faut aller une super-classe plus haut dans la hiérarchie pour atteindre \ct{Class}.
...
\item La méta-classe de \ct{Metaclass} est une instance de \ct{Metaclass}.
	La relation \emph{instance-de} forme une boucle fermée, donc \lct{Metaclass class class}{}\mbox{\ct{--> Metaclass}.}
\end{enumerate}
