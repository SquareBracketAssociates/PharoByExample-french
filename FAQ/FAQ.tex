...
O\`u puis-je trouver la derni\`ere version de Pharo?
\end{faq}
\answer
\pharoweb
\index{download}
...
\end{faq}
\answer
\arelire{Vous pouvez utiliser n'importe quelle image de \pharo mais nous vous
...
\end{faq}
\answer
\arelire{Vous utilisez probablement une image disposant d'une version
...
défaut.
Vous pouvez changer cela en \clickant sur la \arevoir{bulle grise}
...
\emph{choisissez le nouveau Browser par défaut}). Dans la liste de
navigateurs proposée, \clickz sur O2PackageBrowserAdaptor. 
...
\short{b} (pour \emph{browse} \cad parcourir \`a l'aide du navigateur
de classes) sur le nom de la classe ou \short{f} dans le panneau des
cat\'egories du Browser. % REVOIR
...
Charger le paquetage AST depuis \url{http://squeaksource.com}. Ensuite \'evaluer:
\begin{code}{}
...
(Attention: SmaCC-Development est destin\'e \`a la version 3.8 de \squeak)
