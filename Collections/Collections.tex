...
% relecture : Rene Mages (12 jan 2010) 
% relecture : Rene Mages ( 9 aug 2010) 
% relecture : Rene Mages (15 apr 2011)
% relecture : Rene Mages (12 jun 2011)
% - oscar - 28661 - $Date: 2009-08-28 11:31:14 +0200 (Fri, 28 Aug 2009)$
...
\caption{Certaines des classes majeures de collections de \pharo.}
...
octets \lct{Byte}; tout comme un \clsind{IntegerArray} n'a que des entiers \ct{Integer}s et qu'un \clsind{FloatArray} ne peut contenir que des réels à virgule flottante de classe \ct{Float}.
...
des éléments du \ct{Bag} et pour valeurs leur nombre d'occurrences.
  \clsindex{Bag}
  \item Les listes chaînées LinkedList utilisent une
représentation standard simplement chaînée.
  \clsindex{LinkedList}
  \item Les intervalles \ct{Interval} sont représentés 
...
\noindent
La représentation (affiché grâce à la méthode \mthind{Interval class}{printString}) de cet intervalle nous révèle que la classe \ct{Number} (représentant les nombres) dispose d'une méthode de convenance appelée \mthind{Number}{to:} (dans le sens de l'expression ``jusqu'à'') pour générer les intervalles:
...
signalant que les couleurs devraient être triées selon leur
...
pour les collections indexées (\mbox{\ct{Array},} 
\mbox{\ct{OrderedCollection},} \mbox{\ct{SortedCollection}),}
la méthode \mthind{SequenceableCollection}{doWithIndex:} 
...
Quand le message \mthind{Dictionary}{do:} est envoyé à un dictionnaire,
...
En utilisant le message \ct{do:}, nous devons écrire le code suivant :
...
\noindent
Le message \mthind{Collection}{collect:} exécute son bloc-argument
...
Le message \mthind{OrderedCollection}{detect:} renvoie le premier
...
% A la mémoire du mathématicien Gauss,
%Following Gauss,
...
\begin{code}{@TEST}
Smalltalk allClasses count: [:each | '*Collection*' match: each asString ] --> 30
\end{code} %CHANGE
...
\end{code}
\noindent
Ici la variable \ct{collection} ne contient pas la collection nouvellement créée mais le dernier nombre ajouté.
...
Un \ct{IdentityDictionary} utilise l'identité entre objets pour comparer les clés. Il est plus approprié que des \ct{Symbol}s soient utilisés comme clés \arevoir{ou bien que la correspondance soit établie entre les références d'objets et les valeurs.}
...
Pour des applications plus avancées, vous aurez besoin du paquetage d'expressions régulières \arevoir{Regex}.
