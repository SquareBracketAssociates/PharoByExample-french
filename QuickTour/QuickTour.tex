...
fonctionneront. Il n'est donc pas nécessaire de mettre à jour \pharo\,---\,même si nous vous recommandons le télécharger de l'image fournie avec le livre pour un apprentissage étape par étape afin de ne pas être surpris par quelques différences dans l'apparence ou le comportement que nous décrirons.
...
Cette dernière archive contient trois fichiers importantes: l'image, le fichier \emph{sources} et le fichier \emph{changes}. Avec la machine virtuelle, vous devriez donc avoir quatre fichiers tel que nous pouvons le voir sur \figref{download}. \pharo{} se compose ainsi:
...
appelée \textit{Pharo.exe} ou \textit{SqueakVMblabla}~\footnote{Basé sur \sq{}, \pharo utilise par défault une machine virtuelle similaire. A l'avenir d'autres machines pourrait être disponible. Attention toutefois à ne pas essayer la machine virtuelle Cog qui ne fonctionne pas avec l'image du livre.}.
} % CHANGE 29247
