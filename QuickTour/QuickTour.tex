...
machine prête à l'emploi pour suivre ce chapitre. 
...
\caption{Les fichiers à télécharger de \pharo \arelire{pour une des
    plateformes supportées}. \figlabel{download}}
\end{figure}

\begin{enumerate}

\item La \emphind{machine virtuelle} (abrégée en VM pour
...
pouvez voir que la machine virtuelle pour la plateforme choisie   
avez par exemple l'icône de la machine virtuelle pour le système est
appelée \textit{Pharo.exe}.} % CHANGE 29247
...
\seeindex{VM}{machine virtuelle}
  ...
  \item Le fichier \emphind{source} contient le code source du système
    ...
    \emph{SqueakV39.sources}\footnote{\arelire{\pharo est
dérivé de \squeak{} 3.9 et partage actuellement la machine virtuelle
avec \squeak.}}
.
%ajout fr index
...
\item L'\emphind{image} \emph{système}  est un cliché d'un système
...
\arelire{Dans \figref{download}, ces fichiers sont appelés
  \emph{pharo.image} et \emph{pharo.changes}.}
%ajout fr index
\index{fichier!image}
...
% D'un autre côté, si vous téléchargez \pharo pour la première fois,
...
\arelire{Vous pouvez minimiser les fenêtres (ce qui les masque dans la
  barre de tâches située dans le bas de l'écran) en \clickant{} sur la
  \arevoir{bulle orange}. \Click{} sur la \arevoir{bulle verte}
  entraîne l'agrandissement maximal de la fenêtre.} % CHANGE REVOIR -
                                % martial - je préfère 'barre de
                                % tâches' au terme 'dock'  
...
les préférences de votre système ou le pilote de votre dispositif de
pointage.
...
fenêtre de travail \arelire{et les atomes continuent de rebondir d'où
  ils étaient lorsque vous avez quitté.}
...
en double-\clickant{} dans l'espace de travail à différents points dans
...
sélectionné %selon l'endroit où vous cliquez.
...
Notez que le texte ``hello world''~\footnote{\arelire{NdT: C'est une tradition de la
 ...
 apparaît dans la
fenêtre Transcript (voir \figref{helloworld}).
...
En plus de \menu{do it}, vous aurez noté la présence de
\menu{print it} 
%ajout
...
\`A l'ouverture du Browser, tous les panneaux sont vides excepté
le premier à gauche.
...
\`A d'autres moments, vous pourriez avoir en tête qu'une méthode
...
panneau inférieure vous apprendra \arelire{comment faire}. % CHANGE
...
(remarquez que la notation \ct{StringTest>>#testShout} est la façon dont \st
identifie la méthode de la classe \ct{StringTest}).
