...
% martial TODO: super-send&self-send&super-sending à remplacer par la bonne expression / index
...
Le compilateur Smalltalk est probablement l'un des utilisateurs les plus important des \pragmas. SUnit est un autre outil qui utilise les annotations. SUnit est capable de déterminer la couverture d'une application à partir d'un test unitaire. Il est parfois souhaitable d'exclure certaines méthodes de ce calcul de couverture. C'est par exemple le cas de la méthode \ct!documentation! dans la classe \ct!SplitJointTest! :
...
Un \pragma sait dans quelle méthode il a été défini (en utilisant \ct{method}), le nom de la méthode (\ct{selector}), la classe qui contient la méthode (\ct{methodClass}), le nombre de ses arguments (\ct{numArgs}) et quel littéral à été défini comme argument du \pragma (\ct{hasLiteral:} et \ct{hasLiteralSuchThat:}).
