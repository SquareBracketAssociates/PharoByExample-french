...
\ct!instVarAt:! et \ct!instVarAt:put:! sont des \subind{méthode}{primitive}, \ie qu'elles sont implémentées comme des opérations primitives de la machine virtuelle \pharo.
...
Dans \figref{MethodsAsObjects}, nous voyons que classes et méta-classes ont en commun la super-classe \ct{Behavior}. C'est dans cette super-classe que la méthode \mthind{Behavior}{new} est définie, parmi d'autres \arevoir{méthodes-clé} pour ces classes. \tradalert{martial}{orthographe} % REVOIR
...
Même si nous pouvons ignorer de tels envois de messages spéciaux, il existe un autre problème fondamental qui ne peut pas être surmonter par cette approche: les envois à \self ne peuvent pas être interceptés:
\begin{code}{}
point := 1@2.
...
\end{code}
\noindent
et la méthode \ct{run:with:in:} met à jour la variable \ct{hasRun}, désinstalle le \emph{wrapper} (puisque la couverture a été vérifiée) et \arevoir{refait un envoi du message avec la méthode originale}. % REVOIR martial: bizarre!
