...
% martial TODO: super-send&self-send&super-sending à remplacer par la bonne expression (comme SBE normalement) / revoir: couverture de tests / index / ré-envoie vs renvoie
...
\ct!instVarAt:! et \ct!instVarAt:put:! sont des \ind{méthodes primitives}, \ie qu'elles sont implémentées comme des opérations primitives de la machine virtuelle \pharo.
...
La méthode est implémentée dans \ct{Object} de la manière suivante :
\needlines{9}
\begin{code}{}
...
Les outils de développement \pharo tels le navigateur de code, le débogueur ou l'inspecteur utilisent tous les mécanismes réflexifs que nous avons vu jusqu'à présent.
...
\arelire{% martial: j'ai mis implementors et senders puisqu'ils ont été definis ainsi précédemment
...
\arelire{Ce qui est moins connu est qu'il} existe un certain nombre de méthodes pour faire des requêtes similaires dans le protocole \prot{browsing} dans la classe \ct{SystemNavigation}.
Par exemple, vous pouvez parcourir de manière programmatique tous les \implementors du message \ct{ifTrue:} 
% ajout martial
\arelire{(\ie toutes ses implémentations)}
en évaluant:
...
Dans \figref{MethodsAsObjects}, nous voyons que classes et méta-classes ont en commun la super-classe \ct{Behavior}. C'est dans cette super-classe que la méthode \mthind{Behavior}{new} est définie, parmi d'autres \arevoir{méthodes-clé} pour ces classes. \tradalert{martial}{revoir si methodes-cle ou methodes cles; cf. reforme orthographe} % REVOIR
...
\arelire{Une \ct{MethodReference} est un \emph{proxy} léger pour une méthode compilée qui est utilisée par de nombreux outils.}
...
((BrowserEnvironment new forClasses: (Collection withAllSubclasses))
	...
	open
\end{code}
Dans cet exemple, nous demandons à chaque méthode compilée son arbre d'analyse (Refactoring Browser), dans le but de trouver quels \arelire{messages à \super} diffèrent des sélecteurs de méthodes.  
Regardez le protocole \prot{querying} de la classe \ct{RBProgramNode} pour voir un \arelire{certain nombre d'exemples de ce que nous pouvons demander} aux arbres d'analyse. 
...
\dothis{Maintenant évaluons \ct{3 factorial} dans un espace de travail. Nous devons normalement obtenir une fenêtre de débogage et un explorateur comme on peut le voir dans \figref{exploringThisContext}.}
...
Si maintenant nous parcourons la classe de l'objet exploré (\ie en évaluant \ct{self browse} dans le panneau du bas de l'explorateur), vous aller découvrir que c'est une instance de la classe \lct{MethodContext}, comme \arelire{tous les \senders} \aretirer{(ou \emph{envoyeurs})} dans la chaîne.
...
% ajout vf
\arelire{%
Le commentaire de la méthode dit que ``ce message change le comportement des sous-classes de la classe (qui reçoit ce message) et informe la sous-classe de la nécessité d'implémenter la méthode''.% AJOUT
}
% fin ajout vf
...
\mthindex{Object}{halt}
La façon \st de mettre des points d'arrêts dans un programme consiste à évaluer \ct{self halt} aux endroits correspondants. Ceci va provoquer la réification de \ct{thisContext} et une fenêtre de \ind{débogage} va s'ouvrir sur ce point d'arrêt.
...
Cette fois ci, l'image ne se fige pas. Essayer d'exécuter \ct{OrderedCollectionTest}\,--\,que vous pouvez trouver dans la catégorie \mbox{\scat{CollectionsTests-Sequenceable}.}

Comment cela fonctionne-t-il? Regardons le code de \mbox{\clsmthind{Object}{haltIf:}:}
\begin{code}{}
...
par une des sous-classes de \ct{Object} situées dans \arevoir{le chemin de recherche}? %lookup path?
Il s'avère qu'il s'agit d'une méthode pratique pour construire certains types de comportement très dynamique. Un objet qui ne comprend pas un message peut, par la surcharge de \ct{doesNotUnderstand:}, se retrouver dans une stratégie alternative pour répondre à ce message.
...
Puisque le \emph{proxy} n'implémentera virtuellement aucune méthode de lui-même, tout message qui lui sera envoyé sera capturé par la méthode
 \ct{doesNotUnderstand:}. En implémentant cette dernière, le \emph{proxy} peut alors prendre des mesures spéciales avant de déléguer le message à l'objet réél que cache ce \emph{proxy}.
...
Jetons un \oe il sur une solution proposée pour programmer ceci~\footnote{Vous pouvez télécharger le paquetage \pkg{PBE-Reflection} sur \url{http://www.squeaksource.com/PharoByExample/}}.

Nous définissons \arelire{un \emph{proxy} de journalisation} ou \ct{LoggingProxy} ainsi:
...
Nous interceptons tout simplement les messages non-compris et les imprimerons dans la fenêtre Transcript; puis nous metterons à jour le compteur de messages et transmetterons le message à l'objet réél \ct{subject}.
...
\arelire{Cela a pour effet d'échanger toutes les références dans l'image entre
le \ct{point} et le \emph{proxy}. Plus important encore, la variable d'instance \ct{subject} du \emph{proxy} se réfère désormais au \ct{point}.}
...
\ct{ProtoObject} mais dans \ct{Object} que notre \ct{LoggingProxy} n'hérite pas!
La réponse à cette énigme est que \ct{class} n'est jamais envoyé comme message mais \arevoir{est directement répondu} par la machine virtuelle~\footnote{\ct{yourself} aussi n'est jamais vraiment envoyé.
...
\arevoir{Les sélecteurs ne sont jamais envoyés parce qu'ils sont codés directement par le compilateur et transformés en \emph{bytecodes}} % REVOIR voir comment bien formuler ça
...
Même si nous pouvons ignorer de tels envois de messages spéciaux, il existe un autre problème fondamental qui ne peut pas être surmonter par cette approche: les envois à \self \aretirer{(\self-sends)} ne peuvent pas être interceptés:
\begin{code}{}
point := 1@2.
...
Notre \emph{proxy} a été privé de deux envois à \self dans la méthode \mbox{\ct{rect:}:} 
% idée auto-envoi à \self ??
...
l'autre application le plus courante de l'interception des messages non-compris.
...
\end{code}
Tout message non-compris est capturé ici. Si une variable d'instance avec le même nom que le message exists alors nous pouvons demander à notre classe de compiler un accesseur pour cette variable d'instance et nous pouvons envoyer ce message à nouveau
% ajout vf
...
Supposons que la classe \ct{DynamicAcccessors} a une variable d'instance
(non-initialisée) \ct{x} mais qu'elle ne définit pas d'accesseur.
...
\arelire{%
Ce qui peut paraître en peu surprenant, c'est qu'\emph{aucun objet} ne peut jouer le rôle de méthode compilée. Tout ce qu'il peut faire, c'est répondre à quelques messages importants tels que \ct{runs:with:in:}.}
...
\dothis{Définissez une classe vide \ct{Demo}. Imprimez \ct{Demo new answer42}~\footnote{NdT: \arelire{référence au roman de science-fiction humoristique, le ``Guide du Routard Intergalactique'' par Douglas Adams dans lequel la réponse (\emph{answer}) à la question du \emph{pourquoi de l'univers} est $42$}.} 
%ajout vf
via \menu{print it}
et constatez l'habituelle erreur ``Message Not Understood'' être levée.}
...
\dothis{Évaluez l'expression \lct{Demo methodDict at: \#answer42 put: ObjectsAsMethodsExample new.}
...
La machine virtuelle enverra alors à cet objet une nouveau message \ct{run:with:in:} avec les sélecteur de méthode, arguments et receveur originels comme arguments.
Puisque la classe \ct{ObjectsAsMethodsExample} implémente cette méthode, elle intercepte le message et \arelire{se le délègue à elle-même}. % REVOIR un peu lourd 
...
Les \emph{wrappers}~\footnote{NdT: \arelire{Appelés ainsi parce qu'ils s'enroulent (en anglais, \emph{wrap}) autour des méthodes}.} de méthode sont une technique bien connue pour intercepter les messages~\cite{Bran98a}.
...
Lorsqu'il est désinstallé, la méthode originale est retrouve sa place dans le dictionnaire de méthodes.
...
N'importe comment, le Test Runner de \pharo utilise précisement cette technique pour évaluer la \emph{couverture de tests} (en anglais \emph{test coverage}).
...
La méthode \clsmthind{TestRunner}{collectCoverageFor:} illustre clairement \arelire{l'algorithme de validation % ou vérification ?
...
Après que les tests furent lancés, tous les \emph{wrappers} sont désinstallés.
...
\end{code}
\noindent
et la méthode \ct{run:with:in:} met à jour la variable \ct{hasRun}, désinstalle le \emph{wrapper} (puisque la couverture a été vérifiée) et \arevoir{ré-envoie le message à la méthode originale}. % REVOIR martial: bizarre!
...
une méthode hors de dictionnaire de méthode peut être 
appelée.
...
Le compilateur \st est probablement l'un des utilisateurs les plus important des \pragmas. \sunit est un autre outil qui utilise les annotations. \sunit est capable de déterminer la couverture d'une application à partir d'un test unitaire. Il est parfois souhaitable d'exclure certaines méthodes de ce calcul de couverture. C'est par exemple le cas de la méthode \ct!documentation! dans la classe \ct!SplitJointTest! :
...
Le réflexivité se définit par la faculté d'interroger, examiner et même modifier les méta-objets du système d'exécution tels que de simples objets. Nous avons vu que:
...
\item Toutes les classes \st pointent vers une instance de \ct{MethodDictionary} qui associe les sélecteurs aux méthodes compilées, instances de \ct{CompiledMethod}. Une méthode compilée connaît sa classe (ce qui boucle la boucle).
\item \ct{MethodReference} est version allégée d'un \emph{proxy} pour une méthode compilée, disposant de méthodes de commodités additionnelles; il est utilisé par de nombreux outils \st.
\item \ct{BrowserEnvironment}, partie prenante de l'infrastructure du Refactoring Browser, offre une interface plus raffinée que \ct{SystemNavigation} pour interroger le système \arelire{puisque le résultat d'une requête peut être utilisé comme champ d'une nouvelle requête.} Les interfaces disponibles sont à la fois graphique et programmatique.
...
% ajout vf only
\item \arelire{Les \pragmas apportent à \pharo un moyen d'expression fantastique. Ces annotations sont utilisées par le Test Runner de \sunit pour collecter les données de couverture des tests passés.}
\end{itemize}
