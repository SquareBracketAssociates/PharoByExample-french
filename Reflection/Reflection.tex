...
Comme vous avez déjà pu vous en apercevoir, il existe des raccourcis claviers pour parcourir tous les i\underline{m}plémenteurs (\short{m}) and e\underline{n}voyeurs (\short{n}) d'un message particulier.

Ce qui est moins connu est qu'il existe un certain nombres de méthodes pour faire des requêtes similaires dans le protocole \prot{browsing} dans la classe \ct{SystemNavigation}.
...
\dothis{Changer la définition de \ct{Integer>>>factorial} en insérant l'expression souligné ci-dessous:}
...
Par convention, les méthodes \st qui envoient le message \ct{self subclassResponsibility} sont considérées comme abstraites. But how does \clsmthind{Object}{subclassResponsibility} provide a useful error message indicating which abstract method has been invoked?  Very simply, by asking \ct{thisContext} for the sender.
