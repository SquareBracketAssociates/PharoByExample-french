...
% relecture : Rene Mages (mon,  13 june 2011) 
...
qui est une méthode \subind{méthode}{générique}, envoie le message \mthind{Object}{printOn:} à son receveur. 
...
\cmind{Object}{isKindOf:} répond \ct{true} si la classe du receveur est la même que la classe de l'argument ou de l'une de ses sous-classes.
...
système, envoie \ct{initialize} aux instances nouvellement créées.
...
Nous avons déjà vu des exemples similaires dans le \charef{syntax}:
\begin{code}{@TEST | n |}
n := 2.
3 timesRepeat: [ n := n*n ].
n --> 256
...
donnée  (ceci est quelquefois appelé ``la propriété de
...
%  \item Send \ct{yourself} to get back the receiver at the end of a cascade.
  ...
  \item il faut surcharger \ct{postCopy} pour implémenter correctement la copie de vos objets;

  \item il faut envoye \ct{self halt} pour créer un point d'arrêt;

  \item il faut renvoyer \ct{self subclassResponsibility} pour faire une méthode abstraite;

  \item pour donner la représentation en chaîne de caractères d'un objet \ct{String}, vous devez surcharger \ct{printOn:};

  \item il faut surcharger la méthode générique \ct{initialize} pour instancier correctement vos objets;
...
%  flottants, fractions et entiers.
  ...
  \item ces symboles sont uniques mais que les chaînes de caractères ne le sont pas;
