...
être incluses dans un ensemble de crochets pour former une \expreg
...
dans de nombreuses implémentations d'\expregs.
...
Un caractère correspond à une telle expression si il répond \ct{true}
...
\indexmain{Regex!syntaxe!filtrage aux extrêmités}
\begin{table}[htb]
\centering
	\begin{tabular}{lp{8cm}}
		\toprule
		...
		\lct{{\escape}b} & concorde avec une chaîne vide à une extrêmité du mot \\
		\lct{{\escape}B} & concorde avec une chaîne vide ailleurs qu'à une extrêmité du mot \\
		\lct{{\escape}<} & concorde avec une chaîne vide en début de mot\\
		\lct{{\escape}>} & concorde avec une chaîne vide en fin de mot\\
		\bottomrule
	\end{tabular}
	\caption{Les primitives pour le filtrage aux extrêmités.\tablabel{boundaries}}
\end{table}

\begin{code}{@TEST}
'hello world' matchesRegex: '.*\bw.*' --> true      "!extrêmité! de mot avant w"
'hello world' matchesRegex: '.*\bo.*'  --> false    "pas !d'extrêmité! avant o"
\end{code}
...
précédents mais ils prennent en flux de données comme argument.
...
convaincu qu'un \emph{matcher} d'\expreg natif en \st vaut l'effort de
le garder en vie. Pour les conseils et l'encouragement qui ont rendu
...
Wolinski et tout ce que je n'ai pas rencontré ou entendu mais qui sont
d'accord que ce n'aura pas été une perte de temps.
