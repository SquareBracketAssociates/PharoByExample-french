...
% relecture : par Rene Mages  (Fri 03 June 2011) 
...
menu apparaîtra avec \ct{drawOn:} comme premier élément de
...
implémentant cette méthode puisque \ct{drawOn:} est le message qui est
...
Remarquez les icônes triangulaires jouxtant le nom des méthodes (voir \figref{OBinheritanceBrowser}).
...
utilisée.
Vous naviguez au travers des méthodes d'\subind{méthode}{accès} de toutes les variables
...
\arelire{Vous pouvez trouver des exemples supplémentaires dans la FAQ en annexe à partir de la page~\pageref{faq:navigatesupersends}.}
...
dans le paquetage \ct{Monticello} qui ne sont pas dans la catégorie \lct{Monticello}. 
...
\dothis{Créez un paquetage appelé \scat{PBE-Monticello}, et mettez-y
...
% martial : bonne idée mais ce n'est plus (explore (I) mais Explore Pointers (e)) depuis Inspector (qui est buggé dans PBE1.0) [dans 1.2+, c'est toujours l'"ancien" inspecteur avec (I) pour l'explorer
\dothis{Sélectionnez %\arevoir{\menu{self}} dans le panneau gauche de notre
la plus haut ligne dans le panneau gauche de notre
inspecteur et \arevoir{choisissez % \menu{explore (I)} dans le menu contextuel
...
le code \ct{'readme.txt' suffix} dans notre espace de travail par \menu{print it}.
...
Comme nous pouvons le voir sur la méthode \ct{detect:} plus bas sur la
...
\ct{'readme.txt'} à l'avant-dernière ligne de la méthode et que 
...
\dothis{En restant sur la ligne \ct{detect:ifNone:}, cliquez sur 
...
Si vous cliquez sur le bouton \button{Over} (en français, ``par dessus''),
...
Ici nous passons à \ct{value}\,---\,c'est exactement l'endroit depuis lequel nous avons démarré
...
%ajout vf
Après ce coup pour rien, essayons de parcourir l'exécution autrement. Pour ce faire, nous utiliserons le bouton \button{Into} (en français, ``dedans'') permettant de rentrer dans la méthode pour une exécution pas-à-pas.
...
% ajout vf 
(normalement après sept \clickbtn{}s sur \button{Through}\,--\,si vous êtes perdus vous pouvez toujours redémarrer en \clickant{} sur le bouton \button{Restart}),
nous nous attendons à ce que le bloc \ct{ifTrue:} soit exécuté, mais ce n'est pas le cas:
% ajout vf
si vous \clickz{} encore sur le bouton \button{Through}, vous passerez à \ct{6} comme
...
%ajout vf
\ct{SequenceableCollection>>>first} renvoie le premier élément de la collection donc ici le premier caractère de la chaîne de caractères et ainsi, \ct{dot} correspond bien au caractère \ct{.} désormais.
Acceptez la modification via l'option \menu{accept} du menu contextuel.}
...
L'évaluation de l'expression \ct{'readme.txt' suffix} sera complète et affichera la réponse \ct{'.txt'}.
...
La façon d'éviter ce type de problème est d'écrire un test \ind{SUnit} pour définir la réponse.
...
\st est un système multitâche: plusieurs processus légers (aussi
