...
Par exemple, à la place de la structure de contrôle conditionnelle \emph{si-alors-sinon}, \st envoie des messages comme \ct{ifTrue:} à des \arevoir{objets de la classe \clsind{Boolean}}.
% Rene propose de traduire :  Boolean objects par objets de la classe \clsind{Boolean} .
...
% Rene : il faudrait que Latex nous autorise à placer la table apres l'enumeration ...
...
Si vous désirez qu'une chaine comporte une apostrophe, il suffira de doubler l'apostrophe, comme dans \ct{'aujourd''hui'}.
...
(la notation entre accolades est particulière aux dialectes \pharo et à \squeak de \st. 
...
\begin{enumerate}
  ...
  \item les déclarations des variables locales (\ie \ct{cr} et
    \ct{count}); 
  \item un nombre quelconque d'expressions séparées par des points; dans notre exemple, il y en a \arevoir{quatre}.
% Rene : en fait ce n'est pas quatre mais trois ( l'erreur est presente dans l'edition anglaise )
\end{enumerate}
